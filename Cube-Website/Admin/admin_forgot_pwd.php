...
if (isset($_GET['clear_otp']) && !empty($email)) {
    $clear_sql = "UPDATE Staff SET Reset_Token = NULL, Token_Expiry = NULL WHERE Staff_Email = ?";
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($stmt === false) {
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                $update_sql = "UPDATE Staff SET Reset_Token = ?, Token_Expiry = ? WHERE Staff_Email = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit_otp'])) {
    ...
    if (!ctype_digit($entered_otp) || strlen($entered_otp) !== 6) {
        ...
        $sql = "SELECT Staff_Email FROM Staff WHERE Staff_Email = ? AND Reset_Token = ? AND Token_Expiry > NOW()";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt === false) {
            $messages[] = "Database error: " . mysqli_error($conn);
        } else {
            ...
            if (mysqli_stmt_num_rows($stmt) > 0) {
                ...
                if ($clear_stmt === false) {
                    ...
                    mysqli_stmt_execute($clear_stmt);
                    mysqli_stmt_close($clear_stmt);
                    echo '<script>window.location.replace("admin_reset_pwd.php?email=' . urlencode($verified_email) . '");</script>';
                    exit();
                }
            } else {
...
<head>
    ...
    <title>CubePro Hub - Admin Forgot Password</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <a href="admin_login.php?clear_otp=1&email=<?php echo urlencode($email); ?>" id="backToLogin">Back to Login</a>
    </div>

    <script>
        // Intercept back button and redirect to admin_login.php only if coming from reset flow
